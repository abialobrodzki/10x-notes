...
import { createPublicLinkSchema, patchPublicLinkSchema } from "./public-links.schemas";
...
describe("patchPublicLinkSchema", () => {
  ...
  it("should coerce string 'true' to boolean", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      // Boolean coercion might work with form data
      expect(typeof result.data.is_enabled).toBe("boolean");
    }
  });
