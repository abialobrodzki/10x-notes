...
import { notesListQuerySchema, createNoteSchema, updateNoteSchema } from "@/lib/validators/notes.schemas";
...
describe("notesListQuerySchema", () => {
  ...
  it("should coerce include_shared string to boolean true (any non-empty string is truthy)", () => {
    // Note: z.coerce.boolean() coerces any non-empty string to true
    // because strings are truthy in JavaScript
    const validData = { include_shared: "false" };
    const result = notesListQuerySchema.safeParse(validData);
    expect(result.success).toBe(true);
    if (result.success) {
      // "false" string is truthy, so coerces to true
      expect(result.data.include_shared).toBe(true);
    }
  });
});
